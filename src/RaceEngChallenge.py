...
async def run_studies_in_worksheet_rows( 
    session: Session,
    worksheet_id: str,
    row_names: List[str],
    sim_version: str,
    row_suffix: str,
    exploration_config_id: str = None
):
    """Runs studies in specified worksheet rows and updates the worksheet.

    Args:
        session: Authenticated Canopy session object
        worksheet_id: ID of the target worksheet
        row_names: List of row names to run studies on
        exploration_config_id: Exploration configuration ID to add
    ...
    Raises:
        ValueError: If invalid exploration config ID is provided
    """
    ...
    # Create worksheet outline
    outline = NewWorksheetDataOutline(
        rows=worksheet.outline.rows,
        label_definitions=worksheet.outline.label_definitions
    )

    # Process target rows
    modified_rows = []
    for original_row in outline.rows:
        if original_row.name in row_names:
            # If exploration config is not provided, run studies with existing row configs
            if exploration_config_id is None:
                new_row = await get_updated_worksheet_row_after_running_study_with_existing_row_configs(
                    session=session,
                    tenant_id=tenant_id,
                ...
                # If exploration config is provided, run studies with the given exploration
                # and existing row configs
                 # Load and validate exploration config
                exploration_config = await canopy.load_config(session, exploration_config_id)
                if exploration_config.document.sub_type != "exploration":
                    raise ValueError(f"Configuration {exploration_config_id} is not an exploration type")
                
                print(f"Using exploration config: {exploration_config.document.name} ({exploration_config_id})")
                # Create a new row with the exploration config
                new_row = await get_updated_worksheet_row_after_running_study_with_given_exploration(
                    session=session,
                    tenant_id=tenant_id,
                    original_row=original_row,
    ...
    outline.rows.extend(modified_rows)
